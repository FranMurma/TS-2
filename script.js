    ...
    if (playButton) {
        ...
        setTimeout(() => {
            document.querySelectorAll("#playMenu .play-option").forEach(option => {
                ...
                if (translations[localStorage.getItem("language") || "en"][key]) {
                    option.textContent = translations[localStorage.getItem("language") || "en"][key];
                } else {
                    console.warn(`⚠️ No se encontró la traducción para: ${key}`);
        ...
        function positionPlayMenu() {
            ...
            if (isMobile) {
                ...
                playMenu.style.width = `${rect.width}px`;
            } else {
